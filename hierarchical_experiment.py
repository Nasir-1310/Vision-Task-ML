"""
STEP 3: Hierarchical Classification Experiment
Two-stage architecture with label-aware flip augmentation
Expected accuracy: ~95-96%

Architecture:
- Shared backbone: EfficientNet-B2
- Region head: 4 classes (ear, nose, throat, vc)
- Side head: 3 classes (left, right, none)
- Final class: Combination of region + side
"""
...
# Combine predictions to final class
def combine_to_class(region_idx, side_idx, region_probs=None, side_probs=None):
    region = REGION_CLASSES[region_idx]
    side = SIDE_CLASSES[side_idx]
    
    if region == 'ear':
        return class_to_idx['ear-left'] if side == 'left' else class_to_idx['ear-right']
    elif region == 'nose':
        return class_to_idx['nose-left'] if side == 'left' else class_to_idx['nose-right']
    elif region == 'throat':
        return class_to_idx['throat']
    elif region == 'vc':
        # For vc, need to distinguish closed vs open - use a separate head or proxy
        # For now, we'll need the original full class
        return class_to_idx['vc-closed']  # Will be refined

# Augmentation without flip
...
class HierarchicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Label-aware horizontal flip (only during training)
...
class HierarchicalModel(nn.Module):
    def __init__(self, backbone_name='efficientnet_b2', num_regions=4, num_sides=3, num_classes=7):
        super().__init__()
        # Shared backbone
        ...
        # Full class head (for vc-closed vs vc-open distinction and refinement)
...
for epoch in range(50):
    ...
    for images, region_labels, side_labels, full_labels in train_loader:
        ...
        # Combined loss (primary focus on full class)
...
print(f"\nBest Hierarchical Accuracy: {best_acc:.2f}%")
...
tta_transforms = [
    A.Compose([A.Resize(288, 384), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
    A.Compose([A.Resize(320, 426), A.CenterCrop(288, 384), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
    A.Compose([A.Resize(310, 410), A.CenterCrop(288, 384), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
    A.Compose([A.Resize(300, 400), A.CenterCrop(288, 384), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
]
...
print("Model saved as: best_hierarchical_model.pth")
